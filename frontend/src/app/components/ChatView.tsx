...
export function ChatView({
  ...
  useEffect(() => {
    if (state === "success" && txHash && txHash !== notifiedTxRef.current) {
      notifiedTxRef.current = txHash;
      addTransaction({
        txHash,
        recipientName: resolvedContact?.name ?? "recipient",
        amount: amount ?? "0",
        memo: memo ?? undefined,
      });
      addNotification({
        ...
        body: `₱${amount ?? "—"} sent to ${resolvedContact?.name ?? "recipient"}`,
        onClick: onViewHistory,
      });
    }
  }, [state, txHash, amount, resolvedContact, addNotification]);
  ...
  const handleConfirm = async () => {
    ...
    // Real transaction building/signing/submitting isn't wired up yet
    // so the rest of the flow (success screen, notifications) can be
    // tested. DELETE this whole block once builtTx/publicKey/sign/
    // submitTransaction are actually working end to end.
    if (!builtTx || !publicKey) {
      setConfirming(true);
      setConfirmError(null);
      setTimeout(() => {
        const fakeTxHash = `DEMO${crypto.randomUUID().replace(/-/g, "").toUpperCase().slice(0, 60)}`;
